...
export function agentInstructions(session: string, cli = "npx coherence", agent?: string): string {
  ...
  return [
    "DECISION JOURNAL — Optional durable memory, not a task checklist.",
    "Follow repository policy and its preferred record locations. Journal only information",
    "future work needs that is not already adequately recorded. No entries is legitimate.",
    "Routine edits, debugging, plans, and command failures do not require journal entries.",
    "",
    ...
    "",
    "When useful to future work, record a durable decision and its rationale or rejected alternative:",
    `  ${cli} decide "<what you chose>" --over "<what you rejected>" --because "<why>" ${scope}`,
    "",
    "WHEN A NUMBER SURPRISES YOU, DOUBT THE INSTRUMENT BEFORE THE SUBJECT. A conjecture",
    "can preserve an important unresolved question; routine investigation needs no entry:",
    ...
    "",
    "For defects, prefer regression tests/contracts, issues for unresolved defects, and PR",
    "evidence for repairs. Structured defect records are optional unless explicitly required",
    "or consumed by a concrete investigation or automation. When useful:",
    ...
    "",
    "An experiment record is optional when a concrete investigation needs durable prediction",
    "and outcome tracking; having a multi-step plan alone does not require one:",
    ...
    "Optional journal tools: `blocked` records a useful unresolved impasse; `retract` corrects a record.",
    `  ${cli} blocked "<what you could not do>" --because "<why>" ${scope}`,
    `  ${cli} retract <id> --because "<what refuted it>" ${scope}`,
    "",
    "This does not gate a build or completion. Do not manufacture entries or log maintenance deferral.",
  ].join("\n");
}
